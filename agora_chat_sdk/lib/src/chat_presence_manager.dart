...
class ChatPresenceManager {
  ...
  static const MethodChannel _channel = const MethodChannel(
    '$_channelPrefix/chat_presence_manager',
    JSONMethodCodec(),
  );
  ...
  Future<void> publishPresence(String description) async {
    ...
    Map result = await _channel.invokeMethod(
      ChatMethodKeys.presenceWithDescription,
      req,
    );
    try {
  ...
  Future<List<ChatPresence>> subscribe({
    ...
    Map result = await _channel.invokeMethod(
      ChatMethodKeys.presenceSubscribe,
      req,
    );
    try {
      ChatError.hasErrorFromResult(result);
  ...
  Future<List<String>> fetchSubscribedMembers({
    ...
    Map result = await _channel.invokeMethod(
      ChatMethodKeys.fetchSubscribedMembersWithPageNum,
      req,
    );
    try {
      ChatError.hasErrorFromResult(result);
      List<String> list = [];
      result[ChatMethodKeys.fetchSubscribedMembersWithPageNum]?.forEach((
        element,
      ) {
  ...
  Future<List<ChatPresence>> fetchPresenceStatus({
    ...
    Map req = {'members': members};
    Map result = await _channel.invokeMethod(
      ChatMethodKeys.fetchPresenceStatus,
      req,
    );
    try {
      ChatError.hasErrorFromResult(result);
